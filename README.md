...
07:00 — Der Watchdog-Bot sendet den Server Health Report. 07:20 / 17:20 — Der Aggregator-Bot liefert RSS-Insights samt Scoring.
Der Admin antwortet auf einen Link mit dem Befehl сформируй (Generieren). Der Bot generiert Texte, URL-Slugs, das Enterprise-Cover, Tags und injiziert all dies lautlos in die WordPress-Datenbank.
Der Admin öffnet die "Entwürfe". Die Sprachen (Polylang) sind vom System bereits getrennt worden. Tags und Bilder sind bereits gesetzt.
Der Admin prüft die RU-Version, verknüpft im rechten Panel die fertigen EN/DE-Entwürfe und veröffentlicht den Beitrag.
Der Server formatiert selbstständig den Teaser, übersetzt ihn, tauscht UTM-Links aus und pusht ihn inklusive Coverbild in die Telegram-Kanäle.
Kurze Nachrichten werden durch den Befehl телега (Telega) gebildet — vom Admin kopiert, in den Distributor-Bot eingefügt und direkt verteilt (ohne CMS-Umweg).
SEO Enrichment (Rank Math): Ganz unten in den generierten Entwürfen hinterlässt die KI einen textlichen «SEO-Block» (Focus Keyword, Description bis 160 Zeichen, Image Alt). Der Admin kopiert diese Daten in das Rank Math-Panel und löscht den Textblock aus dem Artikel vor der Veröffentlichung. Interne UTM-Links zum Original (?utm_source=...&utm_medium=referral) sind bereits durch das Skript am Ende des Textes hardcodiert.
...
Proactive Alerting: Watchdog überwacht den RAM via pgrep -f "[a]ggregator.py". Die Verwendung von Regular Expressions (eckige Klammern) ist notwendig, damit sich der Such-Prozess nicht selbst findet (Self-Matching) und Fehlalarme (False Positives) auslöst.
Dokument aktualisiert und genehmigt. Architektur V4.0 in die Produktion überführt.
