...
**⚖️ Human-in-the-loop (KI-Transparenz):**
Wir nutzen KI-Pipelines (NLP-Modelle, maschinelle Übersetzungen) zur Aggregation von Insights und zur Erstellung von Rohfassungen. **Jeder Longread durchläuft jedoch ein strenges manuelles Review durch einen Human Engineer**, bevor er veröffentlicht wird. Dies garantiert höchste architektonische Faktengenauigkeit und erfüllt die Transparenzanforderungen für KI-generierte Inhalte (B2B-Standards).
...
* **Async Webhook:** Der Admin klickt bei der RU-Version auf "Veröffentlichen" (nach manuellem Review). Ein selbst geschriebener PHP-Hook extrahiert den Teaser, holt die lokalisierten URLs sowie den Bildlink und feuert asynchron ein JSON-Payload an den internen Port `127.0.0.1:5000` ab.
...
2. Der Admin antwortet auf einen Link mit dem Befehl *Gen* (Generieren). Der Bot generiert Texte, URL-Slugs, das Enterprise-Cover, Tags und injiziert all dies lautlos in die WordPress-Datenbank.
...
4. Der Admin prüft die RU-Version, editiert sie **manuell (Review)**, verknüpft im rechten Panel die fertigen EN/DE-Entwürfe und veröffentlicht den Beitrag.
...
6. Kurze Nachrichten werden durch den Befehl *telega* (Teleram) gebildet — vom Admin kopiert, in den Distributor-Bot eingefügt und direkt verteilt (ohne CMS-Umweg).
...
systemctl restart thecoregrid_ai.service          # Neustart KI-Sekretär
systemctl restart thecoregrid_gateway.service     # Neustart Übersetzer/Gateway
systemctl restart thecoregrid_watchdog.service    # Neustart Watchdog
